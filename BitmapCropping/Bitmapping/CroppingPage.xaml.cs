...
namespace Bitmapping
{
    ...
    public sealed partial class CroppingPage : Page
    {
        ...
        private void Accept_AppBarButton_Tapped(object sender, TappedRoutedEventArgs e)
        {

            double originalWidth = sourceImagePixelWidth;
            double originalHeight = sourceImagePixelHeight;

            double ratioX = 310 / originalWidth;
            double ratioY = MaxHeight / originalHeight;

            double ratio = Math.Min(ratioX, ratioY);

            double newHeight = (originalHeight * ratio);
            double newWidth = (originalWidth * ratio);


            double widthScale = canvas.Width / this.sourceImagePixelWidth;
            double heightScale = canvas.Height / this.sourceImagePixelHeight;

            Size size = new Size(this.selectedRegion.SelectedRect.Width / widthScale, this.selectedRegion.SelectedRect.Height / heightScale);
        }
    }
